...
<head>
    ...
    <title>{{ config('app.name', 'SICIP') }}</title>
    ...
    <style>
        ...
        /* Sidebar */
        .sidebar { min-height: 100vh; background-color: var(--cb-black); color: white; transition: all 0.3s cubic-bezier(0.25,0.8,0.25,1); box-shadow: 2px 0 10px rgba(0,0,0,0.1); z-index: 1040; }
        .sidebar-brand { display: flex; align-items: center; justify-content: center; padding: 1.5rem 1rem; background-color: rgba(0,0,0,0.2); }
        .sidebar-brand-icon { color: var(--cb-green); font-size: 1.8rem; margin-right: 10px; }
        .sidebar .nav-link { color: #adb5bd; padding: 10px 20px; display: flex; align-items: center; border-radius: 8px; margin: 2px 12px; font-weight: 500; transition: all 0.2s; font-size: 0.9rem; }
        .sidebar .nav-link:hover { color: white; background-color: rgba(255,255,255,0.07); transform: translateX(4px); }
        .sidebar .nav-link.active { color: white; background-color: var(--cb-green); box-shadow: 0 4px 10px rgba(0,96,57,0.3); }
        .sidebar .nav-link i { font-size: 1.1rem; margin-right: 10px; min-width: 20px; }
        .sidebar-heading { font-size: 0.7rem; text-transform: uppercase; letter-spacing: 1.5px; color: #495057; font-weight: 700; padding: 1rem 1.5rem 0.4rem; }
        .sidebar-divider { border-color: rgba(255,255,255,0.05); margin: 0.5rem 1rem; }

        /* Navbar */
        .top-navbar { height: 65px; background: white; box-shadow: 0 2px 15px rgba(0,0,0,0.04); z-index: 1030; }
        .content-area { width: 100%; display: flex; flex-direction: column; min-height: 100vh; }
        .main-content { flex: 1; }
        .dropdown-menu { border: none; box-shadow: 0 10px 30px rgba(0,0,0,0.1); border-radius: 10px; }
        .dropdown-item:hover { background-color: #f8f9fa; color: var(--cb-green); }

        @media (max-width: 767.98px) {
            .sidebar { position: fixed; left: -260px; width: 260px; }
            .sidebar.show { left: 0; }
        }
    </style>
    @stack('styles')
...
<div class="d-flex">
    ...
    <div class="sidebar d-none d-md-flex flex-column flex-shrink-0" style="width:260px;" id="sidebar">
        <div class="sidebar-brand">
            ...
            <h4 class="mb-0 fw-bold text-white">SICIP</h4>
        </div>
    ...
    <div class="offcanvas offcanvas-start sidebar d-md-none" tabindex="-1" id="mobileSidebar" style="width:260px;">
        <div class="offcanvas-header sidebar-brand border-bottom border-secondary">
            <div class="d-flex align-items-center"><i class="bi bi-shield-check sidebar-brand-icon"></i><h4 class="mb-0 fw-bold text-white">SICIP</h4></div>
        ...
        <nav class="navbar navbar-expand-lg top-navbar px-3 px-md-4">
            <div class="container-fluid">
                <div class="d-flex align-items-center">
                    ...
                    <h5 class="mb-0 fw-bold d-md-none" style="color:var(--cb-green);">SICIP</h5>
                </div>
