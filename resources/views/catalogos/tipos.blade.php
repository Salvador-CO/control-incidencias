<x-app-layout>
    <x-slot name="header">Tipos de Incidencias</x-slot>
    ...
    <div class="card border-0 shadow-sm rounded-4">
        <div class="card-header bg-white border-bottom py-3 d-flex justify-content-between align-items-center rounded-top-4">
            <h5 class="mb-0 fw-bold"><i class="bi bi-tags me-2 text-success"></i> Tipos de Incidencias</h5>
        ...
        <div class="card-body p-4">
            <div class="table-responsive">
                <table id="tablaTipos" class="table table-hover table-striped align-middle">
                    ...
                    <tbody>
                        ...
                        <tr id="row-t-{{ $t->id }}">
                            ...
                            <td><span class="badge {{ $t->activo ? 'bg-success' : 'bg-secondary' }} rounded-pill px-3">{{ $t->activo ? 'Activo' : 'Inactivo' }}</span></td>
                            <td class="text-center">
                                <div class="btn-group btn-group-sm">
                                    <button class="btn btn-outline-primary" onclick="abrirEditar({{ $t->id }}, '{{ addslashes($t->nombre) }}', {{ $t->activo ? 1 : 0 }})"><i class="bi bi-pencil"></i></button>
                                    <button class="btn btn-outline-danger" onclick="eliminar({{ $t->id }})"><i class="bi bi-trash"></i></button>
                                </div>
                            </td>
                        </tr>
                        @endforeach
    ...
    <div class="modal fade" id="modalCrear" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow-lg rounded-4">
                <div class="modal-header border-0 pb-0"><h5 class="modal-title fw-bold"><i class="bi bi-tags me-2 text-success"></i> Nuevo Tipo de Incidencia</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
                <div class="modal-body pt-3">
                    <label class="form-label fw-bold">Nombre <span class="text-danger">*</span></label>
                    <input type="text" id="nombreCrear" class="form-control form-control-lg" placeholder="Ej. Retardo, Inasistencia...">
                </div>
    ...
    <div class="modal fade" id="modalEditar" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow-lg rounded-4">
                <div class="modal-header border-0 pb-0"><h5 class="modal-title fw-bold"><i class="bi bi-pencil-square me-2 text-primary"></i> Editar Tipo</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
                <div class="modal-body pt-3">
    ...
    <script>
        ...
        $(document).ready(function(){ $('#tablaTipos').DataTable({ language: { url: 'https://cdn.datatables.net/plug-ins/1.13.7/i18n/es-MX.json' } }); });
        ...
        async function guardarNuevo(){
            ...
            const r=await fetch('{{ route("catalogos.tipos.store") }}',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':CSRF},body:JSON.stringify({nombre})});
        ...
        async function guardarEdicion(){
            ...
            const r=await fetch(`/catalogos/tipos/${id}`,{method:'PUT',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':CSRF},body:JSON.stringify({nombre,activo})});
        ...
        async function eliminar(id){
            ...
            const r=await fetch(`/catalogos/tipos/${id}`,{method:'DELETE',headers:{'X-CSRF-TOKEN':CSRF}});
            const d=await r.json(); if(r.ok){showToast(d.message);document.getElementById(`row-t-${id}`)?.remove();}else showToast(d.message,'danger');
        }
    </script>
</x-app-layout>
